#version 300 es

precision highp float;

in vec2 vUV;
out vec4 outColor;

uniform float uAspectRatio;

void main() {
    // Center the coordinates to (-0.5, 0.5) range
    vec2 centeredUV = vUV - 0.5;
    
    // Account for aspect ratio to make the vortex circular
    centeredUV.x *= uAspectRatio;
    
    // Calculate the vortex velocity
    float vx = -centeredUV.y;
    float vy = centeredUV.x;
    
    // Reduce velocity strength for stability
    float strength = 0.5;
    vx *= strength;
    vy *= strength;
    
    // IMPORTANT: Remap from [-0.5, 0.5] to [0, 1] for storage
    // This prevents negative values from being clamped
    vx = vx * 0.5 + 0.5;
    vy = vy * 0.5 + 0.5;
    
    // Store the 2D vector in the R and G channels
    outColor = vec4(vx, vy, 0.0, 1.0);
}